...
def main() -> int:
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != fastapi_icontract.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in crosshair/__init__.py, "
                f"but found: {fastapi_icontract.__status__}"
            )
            success = False
